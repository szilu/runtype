...
describe('test validateSync()', () => {
	...
	describe('test error path reporting', () => {
		it('should report the field path in a struct', () => {
			...
			expect(t.validateSync(tS, { s: 'a' })).toEqual(t.err([{ path: ['s'], error: 'length must be at least 2' }]))
		})

		it('should report the index path in an array', () => {
			const tA = t.array(t.string.minLength(2))
			expect(t.validateSync(tA, ['ab', 'a'])).toEqual(t.err([{ path: ['1'], error: 'length must be at least 2' }]))
		})

		it('should report the key path in a record', () => {
			const tR = t.record(t.string.minLength(2))
			expect(t.validateSync(tR, { k: 'a' })).toEqual(t.err([{ path: ['k'], error: 'length must be at least 2' }]))
		})

		it('should report every failing array element', () => {
			const tA = t.array(t.string.minLength(2))
			expect(t.validateSync(tA, ['a', 'ab', 'b'])).toEqual(t.err([
		...
		it('should report every failing record value', () => {
			const tR = t.record(t.string.minLength(2))
			expect(t.validateSync(tR, { a: 'x', b: 'ok', c: 'y' })).toEqual(t.err([
		...
		it('should report the index path in a tuple', () => {
			...
			expect(t.validateSync(tT, ['a', 1])).toEqual(t.err([{ path: ['1'], error: 'must be at least 10' }]))
		})

		it('should report nested paths', () => {
			const tS = t.struct({ a: t.array(t.struct({ n: t.number.min(10) })) })
			expect(t.validateSync(tS, { a: [{ n: 42 }, { n: 1 }] }))
				.toEqual(t.err([{ path: ['a', '1', 'n'], error: 'must be at least 10' }]))
		})
		...
		it('should report validator errors in a union member', () => {
			...
			expect(t.validateSync(tU, 1)).toEqual(t.err([{ path: [], error: 'must be at least 10' }]))
		})
		...
		it('should report validator errors in a tagged union member', () => {
			...
			expect(t.validateSync(tTU, { type: 'num', n: 42 })).toEqual(t.ok({ type: 'num', n: 42 }))
			expect(t.validateSync(tTU, { type: 'num', n: 1 })).toEqual(t.err([{ path: ['n'], error: 'must be at least 10' }]))
		})
		...
		it('should report validator errors in an intersection', () => {
			...
			expect(t.validateSync(tI, 1)).toEqual(t.err([{ path: [], error: 'must be at least 10' }]))
		})

		it('should report validator errors in an intersection of structs', () => {
			const tI = t.intersection(t.struct({ a: t.number.min(10) }), t.struct({ b: t.string.minLength(2) }))
			expect(t.validateSync(tI, { a: 42, b: 'ab' })).toEqual(t.ok({ a: 42, b: 'ab' }))
			expect(t.validateSync(tI, { a: 1, b: 'ab' })).toEqual(t.err([{ path: ['a'], error: 'must be at least 10' }]))
		})
		...
		it('should report validator errors through lazy', () => {
			...
			expect(t.validateSync(tL, { n: 1 })).toEqual(t.err([{ path: ['n'], error: 'must be at least 10' }]))
		})
		...
		it('should report validator errors through optional', () => {
			...
			expect(t.validateSync(tO, 1)).toEqual(t.err([{ path: [], error: 'must be at least 10' }]))
		})
		...
		it('should report validator errors through nullable', () => {
			...
			expect(t.validateSync(tN, 1)).toEqual(t.err([{ path: [], error: 'must be at least 10' }]))
		})
		...
		it('should report validator errors through default', () => {
			...
			expect(t.validateSync(tD, 1)).toEqual(t.err([{ path: [], error: 'must be at least 10' }]))
		})
	...
	describe('test addAsyncValidator()', () => {
		it('should accept a valid value under validate()', async () => {
			const tAsync = t.number.addAsyncValidator(async v => v === 42 ? t.ok(v) : t.error('must be 42'))
		...
		it('should run both sync and async validators under validate()', async () => {
			const tBoth = t.number.min(0).addAsyncValidator(async v => v <= 100 ? t.ok(v) : t.error('must be at most 100'))
			expect(await t.validate(tBoth, 42)).toEqual(t.ok(42))
			expect(await t.validate(tBoth, -1)).toEqual(t.err([{ path: [], error: 'must be at least 0' }]))
			expect(await t.validate(tBoth, 420)).toEqual(t.err([{ path: [], error: 'must be at most 100' }]))
		})
	...
	describe('test remaining scalar types', () => {
		it('should validate a date', () => {
			expect(t.validateSync(t.date, new Date('2024-01-01'))).toEqual(t.ok(new Date('2024-01-01')))
	...
	describe('test validators attached to wrapper types', () => {
		it('should run a validator attached to an optional wrapper for present values', () => {
			const tO = t.optional(t.string).addValidator(v => v === 'forbidden' ? t.error('forbidden') : t.ok(v))
		...
		it('should run a validator attached to a nullable wrapper for present values', () => {
			const tN = t.nullable(t.number).addValidator(v => v === 0 ? t.error('zero not allowed') : t.ok(v))
			expect(t.validateSync(tN, 42)).toEqual(t.ok(42))
	...
	describe('test async validator guard through nested types', () => {
		...
		it('should throw when a union member that matches has async validators', () => {
			const tU = t.union(t.string.addAsyncValidator(async v => t.ok(v)), t.number)
		...
		it('should throw when a deeply nested type has async validators', () => {
			const tDeep = t.struct({ items: t.array(t.struct({ s: t.string.addAsyncValidator(async v => t.ok(v)) })) })
			expect(() => t.validateSync(tDeep, { items: [{ s: 'a' }] })).toThrow(t.AsyncValidatorError)
		})
	})
})
